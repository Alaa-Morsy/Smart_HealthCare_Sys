...
def calc_hr_and_spo2(ir_data, red_data):
    ...
    if n_peaks >= 2:
        for i in range(1, n_peaks):     #####################################################################
            ...
            RR_list.append(hr)         #Append to list##########################################################
        bpm = 60000 / np.mean(RR_list) #60000 ms (1 minute) / average R-R interval of signal##################
        print ("Average Heart Beat is: %.01f") %bpm #Round off to 1 decimal and print
        hr_valid = True 
    else:
        bpm = -999  # unable to calculate because # of peaks are too small
        hr_valid = False
...
################################################################################################################
    ...
    for i in range(exact_ir_valley_locs_count):
        if ratio_ave > 2 and ratio_ave < 184:
            ...
            spo2 = -999
            spo2_valid = False
    spo2_avg = np.mean(RR_list)   
    return bpm, hr_valid, spo2_avg, spo2_valid  #############################################################
